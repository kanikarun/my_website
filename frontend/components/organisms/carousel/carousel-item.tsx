...
export const CarouselItem: React.FC<CarouselItemProps> = props => {
  ...
  return (
    <div className="relative min-h-dvh">
      ...
      <div className={cn('absolute bg-navy-blue mix-blend-multiply size-full', { 'bg-purple': isOdooTheme })} />
      <div className="isolate container flex flex-col min-h-dvh pt-24 pb-16 space-y-16">
        <div className="grow flex items-center">
          <div className="space-y-6">
            ...
            <h1 className="block font-black text-white text-3xl lg:text-5xl lg:leading-tight">{title}</h1>
            <p className="text-gray-300 sm:text-base md:max-w-2xl md:text-lg">{subtitle}</p>
            <div className="flex gap-4 flex-wrap">
              {buttons?.map((x, i) => (
                <Button
                  key={`button-${i}`}
                  className={cn('grow sm:grow-0', { 'bg-white text-purple hover:bg-white/80': isOdooTheme && !i })}
                  size="2xl"
...
const Metrics: React.FC<{ data?: MetricProps[] }> = ({ data }) => {
  ...
  return (
    <div className="text-white flex lg:justify-evenly items-end flex-wrap lg:gap-0 gap-y-10">
      {data.map(x => {
        if (x.type === 'image') return <StatImage key={x.label} label={x.label} src={x.value} />;
...
const StatContainer: React.FC<StatContainerProps> = ({ children, label, className }) => {
  return (
    <div className={cn('px-4 size-full flex flex-col basis-1/2 lg:basis-1/4 nth-[2n-1]:border-r lg:not-last:border-r', className)}>
      {children}
...
const StatText: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <StatContainer label={label}>
    <p className="text-3xl sm:text-4xl text-center">{value}</p>
  </StatContainer>
);

const StatImage: React.FC<{ label: string; src: string | StaticImport }> = ({ label, src }) => (
  <StatContainer label={label} className="space-y-2 size-full items-center">
    <NextImage className="object-contain" src={src} alt={label} width={120} height={90} />
  </StatContainer>
);
