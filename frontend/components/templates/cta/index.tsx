...
export const TemplateCTA = (props: TemplateCTAProps) => {
  ...
  return (
    <div className="py-16 relative">
      ...
      <div
        className={cn('absolute top-0 size-full bg-primary mix-blend-multiply', {
          'bg-purple dark:bg-purple': isOdooVariant
        })}
      />
      <div className="container isolate grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <h3 className="text-2xl font-black md:text-3xl text-white">{title}</h3>
        ...
        <div className="flex gap-3 flex-col lg:ml-auto">
          {buttons.map((item, i) => (
            <Button
              ...
              className={cn('lg:min-w-xs', { 'text-purple': isOdooVariant && !i })}
            >
