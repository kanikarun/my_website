...
export const NavigationBar: React.FC<Props> = ({ navigation }) => {
  ...
  return (
    <header
      className={cn("w-full", {
        "absolute z-50": isMatch,
        "bg-white dark:bg-black": !isMatch,
      })}
    >
  ...
  return (
    <div className="container hidden py-4 items-center justify-between xl:flex">
      <Link href={ROUTES.HOME}>
        {[
          { src: getStrapiMedia(navigation?.logo), className: 'block dark:hidden' },
          { src: getStrapiMedia(navigation?.logo_dark), className: 'hidden dark:block' },
        ].map(({ src, className }) => (
          <Image key={className} src={src} alt="OneWorld Technology" width={160} height={40} className={className} />
        ))}
      </Link>
        ...
        <div className="flex items-center justify-between">
          <Link href={ROUTES.HOME}>
            {[
              ...
              <Image key={className} src={src} alt="OneWorld Technology" width={160} height={24} className={className} />
            ))}
          </Link>
          <div className="flex items-center space-x-4">
            <LocaleToggle />
