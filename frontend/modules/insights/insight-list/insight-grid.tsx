...
export const InsightGrid: React.FC<Props> = ({ initialData, locale }) => {
  ...
  const dataLength = q.data?.pages.reduce((acc, page) => acc + page.data.length, 0) ?? 0;

  if (!results.length) return <EmptyState />;

  return (
    <XInfiniteScroll dataLength={dataLength} next={q.fetchNextPage} hasMore={!!q.hasNextPage}>
      <div className="container grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 py-1">
        {results.map(x => (
          <ArticleCard
            ...
            btnLink={ROUTES.INSIGHT_DETAIL(x.slug)}
          />
        ))}
      </div>
    </XInfiniteScroll>
