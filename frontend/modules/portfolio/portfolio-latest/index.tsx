...
export const PortfolioLatest: React.FC<PortfolioLatestProps> = async (
  props,
) => {
  const { sectionTitle, locale } = props;
  const [t, { data }] = await Promise.all([
    getTranslations("components.portfolio"),
    getPortfolios({ page: 1, pageSize: 3, locale }),
  ]);
  ...
  return (
    <div className="dark:bg-black py-8 lg:py-16">
      <div className="container  space-y-9 flex flex-col">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 not-first-of-type:gap-6 items-stretch">
          {data.map((x) => (
            <ArticleCard
              {...x}
              key={x.id}
              btnText={t("btn-detail")}
              btnLink={ROUTES.PORTFOLIO_DETAIL(x.id, x.title)}
            />
          ))}
        </div>
